...
作者頁 Hero 目前使用「研究象徵圖像」（symbolic research avatar），**不是**作者照片，頁面上也如此標示。

1. 圖檔路徑：`public/images/author/research-avatar.jpg`（建議直式，約 3:4 比例，至少 480×640）。
2. 上傳方式：GitHub 網頁進入該資料夾 →「Add file」→「Upload files」。
3. 圖檔不存在時會顯示姓名縮寫替代卡，不會破圖。
4. 圖說標籤與 alt 文字集中在 `content/author.ts` 的 `avatar` 欄位。

> ⚠️ **授權注意**：目前作者頁使用之研究象徵圖像為暫時素材，正式長期公開前應確認授權或替換為原創圖像。（該圖像取材自具著作權之動漫角色意象，長期公開有侵權風險。）

此圖像**不會**寫入 Schema.org 的 `Person.image`，metadata 中也不描述為作者本人照片。若日後要放真正的作者照片，請先與維護者（或 Claude Code）確認要新增獨立的照片欄位，不要直接覆蓋象徵圖像。
